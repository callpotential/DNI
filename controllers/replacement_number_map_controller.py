...
@trace_logging()
def get_replacement_map_item_with_number_to_replace(number_to_replace: PhoneNumber):

    sql = "SELECT * FROM replacementnumbermap WHERE replacementphonenumber = '" + str(number_to_replace) + "'"
...
def insert_replacement_map(replacement_map: ReplacementNumberMap):
    sql = "INSERT INTO replacementnumbermap ( replacementphonenumber, routingnumber, poolid ) " \
          "VALUES ( '" + str(replacement_map.replacementphonenumber) + "', '" + str(replacement_map.routingnumber) + "', '" + str(replacement_map.poolid) + "' );"
